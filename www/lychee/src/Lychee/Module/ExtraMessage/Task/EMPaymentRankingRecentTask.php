...
class EMPaymentRankingRecentTask implements Task {
	...
	public function getName() {
		return 'extramessage_payment_ranking_recent';
	}
	...
	public function run() {
		...
		for($i=1;$i<=$totalStoryId;$i++){
			...
			$storyId = $i;
			$results = $this->extraMessageService->getRecentPaymentRanking($storyId, 100);

			$returnResult = array();
			foreach($results as $productPurchased){

				/** @var PaymentProductPurchased $productPurchased */
				$paymentComment = $this->extraMessageService->getPaymentComment($productPurchased['purchase_id']);
			...
			$this->memcache->set('cache_extramessage_payment_ranking_recent_story_' . $storyId, $returnResult);
		}
