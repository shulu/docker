(function ($) {
    ...
    $.paginator = function (el, options) {
        ...
        self.$container.data('paginator', self);
        ...
        self.init = function () {
            ...
            self.options = $.extend({}, $.paginator.defaultOptions, options);
        ...
        self.renderHtml = function () {
            ...
            if (self.options.wrapper) {
                self.$container.html($(self.options.wrapper).html(html.join('')).paginatorHTML());
            } else {
        ...
        self.callMethod = function (method, options) {
            switch (method) {
                ...
                case 'destroy':
                    ...
                    self.$container.removeData('paginator');
                    break;
                default :
                    throw new Error('[paginator] method "' + method + '" does not exist');
            }
    ...
    $.fn.paginator = function () {
        ...
        if (typeof args[0] === 'string') {
            var $instance = $(self).data('paginator');
            if (!$instance) {
                throw new Error('[paginator] the element is not instantiated');
            } else {
                return $instance.callMethod(args[0], args[1]);
            }
        } else {
            return new $.paginator(this, args[0]);
        }
    };
